...
int main(int argc, char** argv)
{
	...
	floor->AddComponent(new RigidBodyComponent(floor, ComponentType::STATIC, 1));
	sandbox.AddEntity(floor);
	entity::Rectangle* leftWall = new entity::Rectangle(wallSize / 2.0f, HEIGHT / 2.0f, wallSize, HEIGHT);
	leftWall->AddComponent(new RigidBodyComponent(leftWall, ComponentType::STATIC, 1));
	...
	rightWall->AddComponent(new RigidBodyComponent(rightWall, ComponentType::STATIC, 1));
	sandbox.AddEntity(rightWall);

	entity::Rectangle* rectangle = new entity::Rectangle(25.0f, 25.0f, 50.0f, 50.0f);
	rectangle->AddComponent(new RigidBodyComponent(rectangle, ComponentType::DYNAMIC, 1));
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		circle->AddComponent(new RigidBodyComponent(circle, ComponentType::DYNAMIC, 1));
